...
import { GetFeatureArtists } from "shared/services/generalService";
import styles from "./style.module.scss";
import NoContentCard from "../noContentCard";
import { useNavigate } from "react-router";
import { routeConstant } from "shared/routes/routeConstant";

const FeatureArtistsCard = () => {
  ...
  return (
    featureArtists?.length > 0 ? <div
      className={classNames(
        "d-flex flex-column align-items-center justify-content-center pt-5 pb-4 position-relative"
      )}
    >
      {loading ? (
        <>
          ...
          <div
            className={classNames(
              "d-flex flex-column align-items-start justify-content-start w-100 gap-1 mt-3"
            )}
          >
        ...
        <>
          <div className={classNames("w-100")}>
            <Slider {...settings} ref={sliderRef}>
              {featureArtists?.map((item: any, key) => {
                return (
                  <div
                    ...
                    onClick={() => {
                      navigate(
                        routeConstant.profile.path.replace(
                          ":id",
                          featureArtists[activeIndex]?.id
                        )
                      );
                    }}
                  >
                    ...
                    >
                      ...
                      />
                      <label className={classNames(styles.featureTitle)}>
                        Featured
                      </label>
                    </div>
                  </div>
          ...
          <div
            className={classNames(
              ...
              featureArtists?.length > 3
                ? "justify-content-start"
                : "justify-content-center"
            )}
            id="featureDotsContainer"
          >
            {featureArtists?.map((item, key) => {
              return (
                <div
                  className={classNames(
          ...
          >
            <label className={classNames(styles.title)}>Featured Artist</label>
            <label
            ...
            >
              {featureArtists[activeIndex]?.name}
            </label>
          </div>
        </>
      ) : (
...
export default FeatureArtistsCard;
