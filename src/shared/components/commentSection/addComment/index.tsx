...
import mentions from "./mention.module.css";
...
const AddCommentInput = ({ comment, setComment, action, loading, placeholder, setParentComment, item, setIsReplying }: AddCommentInputProps) => {
  const { id } = useParams();
  ...
  return (
    <div className={classNames("d-flex w-100 align-items-start justify-content-between gap-2 mt-4", item && styles.replyCommentContinaer)}>
      ...
      <div className={classNames("col-2")}>
        <button className={classNames(styles.sendIconContainer, comment?.length > 0 && styles.activeComment)} disabled={comment?.length === 0 || loading} onClick={() => handleMentions(comment)}>
          {loading ? <Spinner animation={"border"} size="sm" style={{ color: "white" }} /> : <Icons.Send className={classNames(styles.sendIcon)} />}
        </button>
      </div>
    </div>
  );
